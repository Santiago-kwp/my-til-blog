...
dst2 = cv2.gemm(src1, src2, alpha, None, beta, flags=cv2.GEMM_2_T) # 두번째 인수를 전치한 후에 행렬의 내적을 계산한다.
dst3 = cv2.gemm(src1, src3, alpha, None, beta) # 행렬을 전치하지 않고 행렬의 내적 을 계산한다.
