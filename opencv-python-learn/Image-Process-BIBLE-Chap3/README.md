...
스타일 전이(Style Transfer)는 한 이미지의 **스타일**을 다른 이미지에 옮겨 적용하는 기술이다. 예를 들어 내가 찍은 사진을 반 고흐 화풍으로 바꾸거나, 일반 사진을 수채화처럼 변환하는 것이 대표적인 예다.

**역사적 흐름**

| 방법 | 설명 | 한계 |
|------|------|------|
| 텍스처 매핑 | 특정 위치에 이미지를 연결 | 다양한 스타일 표현 어려움 |
| 히스토그램 지정 | 명암·색조를 원하는 분포로 변환 | 전반적 분위기만 변환 가능 |
| **신경 스타일 전이** | CNN 중간 층의 특성 추출 활용 | 연산 비용이 높음 |

딥러닝 등장 이후, CNN의 중간 층을 통한 특성 추출을 기반으로 한 **신경 스타일 전이(Neural Style Transfer)** 가 제안되어 훨씬 자연스러운 스타일 변환이 가능해졌다.
...
| 표현 | 의미 | 예시 |
|------|------|------|
| **콘텐츠 표현** | 이미지의 기본 형태와 객체 | 산, 건물, 사람 등 주요 피사체 |
| **스타일 표현** | 그림의 '느낌'과 '방식' | 색상 조합, 붓질, 텍스처 등 |

결국 신경 스타일 전이는 **콘텐츠 추출 모델**과 **스타일 추출 모델**, 그리고 두 특성을 결합하여 이미지를 생성하는 모델의 조합으로 구현된다.

#### VAE를 활용한 잠재 벡터 추출

원하는 특징을 가진 이미지를 생성하려면, 이미지의 특징을 숫자 벡터로 표현하고 그 벡터로부터 이미지를 복원하는 구조가 필요하다. 이 특징 벡터를 **잠재 벡터(Latent Vector, $z$)** 라 부르며, VAE(Variational AutoEncoder)는 이 잠재 벡터를 학습하는 대표적인 모델이다.

> **비유**: 잠재 벡터는 "몽타주 주문서"다. "안경 수치=80, 웃음 수치=90" 같은 특징을 숫자로 압축한 리스트로, 이 주문서를 받아 이미지를 그려내는 것이 **디코더(화가 $\theta$)** 의 역할이다.
...
오토 인코더와 유사한 구조이지만, VAE는 잠재 벡터를 **확률 분포**로 표현한다는 점이 핵심 차이다.

---

#### VAE의 목적 함수 유도 (ELBO)

**최종 목표**: 학습 데이터 $x$가 모델에서 생성될 확률 $p_{\theta}(x)$를 최대화하는 것.

확률값은 매우 작은 소수이므로 곱셈 연산 시 언더플로우가 발생한다. 이를 방지하기 위해 $\log$를 씌워 $\log p_{\theta}(x)$ 최대화 문제로 변환한다.

**1단계 — 계산의 벽: 적분 불가**

$$p_{\theta}(x) = \int p_{\theta}(z)\, p_{\theta}(x|z)\, dz$$

모든 가능한 잠재 벡터 $z$에 대해 적분해야 하는데, $z$의 경우의 수는 무한대이므로 직접 계산이 불가능하다.
...
여기서 $p_{\theta}(z|x)$는 "사진을 보고 완벽한 잠재 벡터를 역추적할 확률"인데, 이 역시 계산이 불가능하다. 대신 **인코더 모델 $q_{\varphi}(z|x)$** 로 근사한다.

$$q_{\varphi}(z|x) \approx p_{\theta}(z|x)$$

**3단계 — 변분 참조 트릭 (핵심)**

이미지는 픽셀 값이 조금만 달라져도 크게 다른 데이터가 된다. 그래서 인코더가 잠재 벡터를 하나의 고정값으로 출력하면 과적합이 발생한다. VAE는 이를 해결하기 위해 잠재 벡터를 **확률 분포에서 샘플링**한다.

$$(\mu,\ \log\sigma) = \text{EncoderNet}_{\varphi}(x)$$

$$q_{\varphi}(z|x) = \mathcal{N}(z;\, \mu,\, \text{diag}(\sigma))$$

인코더는 정확한 값 대신 **"잠재 벡터는 평균 $\mu$, 표준편차 $\sigma$인 정규분포 범위 안에 있다"** 고 알려주고, 그 범위에서 랜덤으로 하나를 뽑아 디코더에 전달한다. 이 랜덤성이 모델의 일반화 능력을 높인다.

**4단계 — 수식 분리: KL 발산 + ELBO**

수식 전개를 거치면 $\log p_{\theta}(x)$는 두 항으로 분리된다.

$$\log p_{\theta}(x) = \underbrace{D_{KL}\!\left(q_{\varphi}(z|x)\,\|\,p_{\theta}(z|x)\right)}_{\geq\, 0} + \underbrace{L(\theta,\varphi;\,x)}_{\text{ELBO}}$$

| 항 | 의미 | 특징 |
|----|------|------|
| $D_{KL}$ | 인코더 근사와 실제 정답의 거리 | 항상 $\geq 0$, 직접 계산 불가 |
| $L(\theta,\varphi;\,x)$ — **ELBO** | 계산 가능한 하한(Lower Bound) | 최대화 대상 |

$D_{KL} \geq 0$ 이므로 $\log p_{\theta}(x) \geq L(\theta,\varphi;\,x)$ 가 항상 성립한다.

**핵심 아이디어**: 목표값 $\log p_{\theta}(x)$를 직접 최대화할 수 없으니, 항상 그 아래에 존재하는 **ELBO를 최대화**하면 목표값도 간접적으로 높아진다.

```mermaid
graph LR
    A["log p(x)\n직접 계산 불가"] -- "항상 ≥" --> B["ELBO L(θ,φ;x)\n계산 가능"]
    B -- "ELBO 최대화" --> C["log p(x) 간접 최대화"]
...
$$L(\theta,\varphi;\,x) = \underbrace{E_{q_\varphi(z|x)}\!\left[\log p_{\theta}(x|z)\right]}_{\text{복원 능력 (높을수록 좋음)}} - \underbrace{D_{KL}\!\left(q_{\varphi}(z|x)\,\|\,p(z)\right)}_{\text{정규화 벌점 (낮을수록 좋음)}}$$

| 항 | 의미 | 직관 |
|----|------|------|
| **복원 능력** | 잠재 벡터로 원본을 얼마나 잘 복원하는가 | 화가가 주문서를 보고 원본과 똑같이 그릴수록 ↑ |
| **정규화 벌점** | 잠재 공간이 표준 정규분포($\mathcal{N}(0,I)$)에서 얼마나 벗어났는가 | 인코더가 표준 양식을 벗어날수록 ↑ (패널티) |

**VAE 학습 목표 요약**:

> 1. **인코더**: 잠재 벡터를 표준 정규분포에 가깝게 표현하라 (정규화)
> 2. **디코더**: 잠재 벡터를 보고 원본 이미지를 최대한 똑같이 복원하라 (복원)

학습 완료 후에는 디코더만 단독으로 사용하여, 잠재 벡터 $z$ 의 값을 직접 조절하면서 원하는 특징을 가진 새로운 이미지를 생성할 수 있다.

---

**한 줄 요약**: VAE는 이미지를 확률 분포(잠재 벡터)로 압축하는 인코더와, 그 분포에서 샘플링한 벡터로 이미지를 복원하는 디코더를 함께 학습시켜, 새롭고 자연스러운 이미지를 생성할 수 있도록 하는 모델이다.
