...
> 스타일 전이(style transfer)는 이름에서도 알 수 있듯이 한 이미지의 '스타일'을 다른 이미지에 '전이'시키는 기술을 의미합니다. 예를 들어 후기 인상주의 작가 반 고흐의 그림 스타일을 좋아한다고 가정해봅시다. 이떄 가지고 있는 사진을 반 고흐 작가의 스타일로 바꿔서 그려보고 싶을 수 있을 것 같습니다. 이러한 일은 아주 많은 노력이 들어가지만, 인공지능을 사용해서 재현해볼 수 있습니다. 이때 사진을 예술가의 그림 스타일로 변환해주는 것이 바로 스타일 전이의 핵심 아이디어입니다.

> 초기의 스타일 전이 방법들은 간단한 텍스처 매핑이나 히스토그램 지정 같은 기술을 사용했습니다. 텍스처 매핑 기법은 3D 이미지 등에서 특정 위치에 이미지를 연결시키는 기법이며, 히스토그램 지정 기법은 이미지의 명암이나 색조를 원하는 스타일로 바꿀 때 사용하는 기법입니다. 이러한 기법들은 특정 상황에는 유용하나, 스타일을 변형시키는 다양성에 있어서는 부족함이 있었습니다. 딥러닝의 등장 후, 합성곱 신경망의 중간 층을 통한 특성 추출을 기반으로 한 신경 스타일 전이가 제안되었습니다.

> 스타일 전이 과정에는 두 가지 핵심 개념이 등장합니다. 바로 스타일 표현(style representation)과 콘텐츠 표현(content representation)입니다. 콘텐츠는 이미지의 기본적인 형태와 객체를 의미하며, 그림 속의 산, 건물, 사람 등 주된 객체나 특성이 여기에 포함됩니다. 한편 스타일은 그림의 '느낌'이나 '방식'을 나타내는 요소로, 색상의 조합, 붓질, 텍스처 등이 여기에 해당합니다. 가지고 있는 사진을 피카소 작가의 작품처럼 변경하고 싶다는 상황에서 필요한 것은, 가지고 있는 사진의 내용(콘텐츠 표현)과 반 고흐 작가의 화풍(스타일 표현)일 것입니다. 즉, 이미지의 내용을 추출할 수 있는 인공지능 모델과 이미지의 스타일을 추출할 수 있는 모델, 두 가지 특성을 결합하여 이미지를 생성할 수 있는 인공지능 모델이 적용되어야 할 것입니다.

#### VAE를 활용한 잠재 벡터 추출

**이미지 생성 모델의 구현 가능성**

> 모델의 구현 가능성 판단은 보통 직관과 수학적 접근으로 이루어집니다. 원하는 특징을 가진 다양한 이미지를 생성하는 인공지능 모델을 만들고 싶다면, 특징 값, 즉 특징 벡터를 입력으로 받아 원하는 특징을 결과 값으로 반환하는 모델 구조를 상상해볼 수 있을 것입니다. 여기서 특징을 표현하는 벡터를 **잠재 벡터(Latent Vector)**라 지칭합니다.

> 잠재적으로 사람이 이미지를 보고 느낀 특징을 표현할 수 있는 벡터들이 있고, 여기에 대해서 원하는 이미지를 생성하는 것이 학습될 수 있다면, 인공지능 모델을 구현해볼 수 있을 것 입니다. 이에 대해 인공지능 연구진들은 간단한 수식적 증명과 함께 인공지능 모델을 구현해냅니다. 원하는 특징에 대한 이미지를 만들기 위해 가장 기본적이면서도 대표적인 모델로 VAE(Variational AutoEncoder)를 생각해볼 수 있습니다.

> VAE의 목적은 '원하는 특징을 가진 이미지를 만들어낼 수 있는가?' 입니다. 그리하여 원하는 특징, 즉 잠재 벡터 z를 받아 이미지를 만들어주는 디코더 모델 $\theta$, 모델 $\theta$에 따른 확률 분포 $p_{\theta}$를 생각할 수 있습니다. 그리고 실제 생성해내고 싶은 이미지 데이터 세트의 분포를 x라고 하면, x와 z의 동시 등장 확률을 의미하는 확률 분포 $p_{\theta}(x, z)$ 값을 최대로 만들어주는 모델 $\theta$와 잠재 벡터 z 를 찾는 것이 현재 모델 구현의 목적이라고 할 수 있습니다. 여기서 데이터 세트의 분포 x는 전체 데이터 세트 X로 부터 관측된 변하지 않는 값이고, 모델 $\theta$와 잠재 백터 z는 찾아내야 할 값입니다. 베이즈 정리에 의하면 아래와 같은 수식이 성립합니다.

$$
p_{\theta}(x,z) = p_{\theta}(z)p_{\theta}(x|z) = p_{\theta}(x)p_{\theta}(z|x)
$$

> 일반적으로 전체 데이터 분포 X를 모두 관측할 수는 없기 때문에 $p_{\theta}(X)$를 알아낼 수 없으며, 때문에 원하는 이미지 x를 만들기 위한 $p_{\theta}(z|x)$도 쉽게 얻어낼 수 없습니다.

> 이를 해결하기 위해 또 한 번(기존의 인공지능 모델을 학습시키는 행위도 일종의 근사이기 때문에)의 근사 기법을 사용합니다.

$$
q_{\varphi}(z|x) \approx p_{\theta}(z|x)
$$

> 바로 원하는 데이터 이미지 x를 만들기 위한 잠재 벡터 z를 찾기 위한 모델 $\varphi$를 하나 더 구성하는 것입니다. 이렇게 모델 $\varphi$가 이미지 x의 특성을 잘 표현하는 이상적인 잠재 벡터 z를 찾아준다면, 우리가 원하는 이미지의 특성을 조절하여 이미지를 변화시켜 볼 수 있을 것입니다. 하지만 여기에는 약간의 문제가 있습니다.

> 바로 픽셀의 집합인 이미지에서는 같은 특성도 크게 다르게 나타나기도 한다는 것입니다. 그래서 각 상황마다 이미지에 무수히 많은 정답 데이터를 연결시켜 놓을 수도 없고, 적절히 모델 $\varphi$가 학습되기도 어렵습니다. 이를 해결하기 위해 변분 참조(variational inference) 트릭이 사용됩니다. 바로 모델 $\varphi$가 만들어낸 결과 값을 참조해서 잠재 벡터 z를 랜덤 추출해내는 방식입니다.

$$
(\mu, log\sigma) = EncoderNeuralNet_{\varphi}(x)
$$

$$
q_{\varphi}(z|x) = N(z; \mu, diag(\sigma))
$$

> 위 식에서 $N(z; \mu, diag(\sigma))$는 평균 $\mu$, 표준편차 $\sigma$를 따르는 정규분포를 뜻합니다. 신비롭게도 이렇게 한 번 랜덤 추출된 벡터는 모델이 데이터의 특성을 좀 더 잘 학습할 수 있게끔 도와줍니다.

> 이렇게 특이하게도 앞서 설명한 오토 인코더와 매우 유사한 구조를 이루게 되며, VAE라는 이름을 갖게 되었습니다. 이러한 구조에서 학습이 진행되면 이미지의 스타일을 적절히 나타낼 수 있는 잠재 벡터 z의 잠재 공간이 도출되며, 학습을 마친 후에는 디코더 모델 $\theta$만 사용하여, 변하는 잠재 벡터 z에 따라 변화하는 이미지를 확인해볼 수 있습니다.

---

#### 1. 잠재 벡터 (Latent Vector, $z$)란 무엇인가?

- **설명:** **"몽타주 주문서"**로 비유할 수 있습니다.
  만약 우리가 인공지능에게 "안경을 쓰고, 머리가 길고, 웃고 있는 사람 그려줘"라고 명령한다면, 이 '안경, 긴 머리, 웃음' 같은 특징들을 숫자로 압축해 놓은 리스트가 바로 **잠재 벡터($z$)**입니다.

#### 2. VAE의 목적 (디코더, $\theta$)

- **설명:** **"주문서를 보고 그림을 그리는 천재 화가($\theta$) 키우기"**입니다.
  우리의 목표는 완벽한 화가(모델 $\theta$)를 만드는 것입니다. 이 화가는 특징 주문서($z$)를 받으면, 그 특징이 쏙 들어간 진짜 같은 사진($x$)을 뚝딱 그려내야 합니다.

#### 3. 수학적인 장벽 (알 수 없는 값들)

- **기본 설명:** 전체 데이터 분포 $X$를 모두 관측할 수 없어 $p_{\theta}(X)$를 알아낼 수 없고, $p_{\theta}(z|x)$도 얻어낼 수 없다.
- **쉬운 설명:** **"완벽한 역추적은 불가능하다"**는 뜻입니다.
  세상의 모든 이미지($X$)를 수학적으로 완벽하게 계산하는 것은 불가능합니다. 따라서 어떤 사진($x$)을 딱 던져줬을 때, "이 사진의 완벽한 특징 주문서($z$)는 이것이다!"라고 수학 공식으로 단번에 계산해 내는 것($p_{\theta}(z|x)$)도 불가능합니다.

#### 4. 해결책 1: 새로운 모델 도입 (인코더, $\varphi$)

- **기본 설명:** $q_{\varphi}(z|x) \approx p_{\theta}(z|x)$, 모델 $\varphi$를 하나 더 구성하는 것.
- **쉬운 설명:** **"사진을 보고 주문서를 작성하는 분석가($\varphi$) 고용하기"**입니다.
  수학 공식으로 완벽한 주문서를 알아낼 수 없으니, **사진을 보고 특징을 요약해 주는 또 다른 인공지능(모델 $\varphi$)**을 만듭니다. 이 분석가는 진짜 사진($x$)을 보고 "음, 이건 안경 수치 80, 웃음 수치 90이군" 하며 주문서($z$)를 작성해 줍니다.

#### 5. 핵심 문제와 해결책 2: 변분 참조 트릭 (랜덤 추출)

- **어려운 설명:** 모델 $\varphi$가 만들어낸 결과 값을 참조해서 잠재 벡터 $z$를 랜덤 추출해내는 방식. 평균 $\mu$, 표준편차 $\sigma$를 따르는 정규분포에서 추출.
- **쉬운 설명:** **"딱딱한 숫자 대신, 유연한 범위(확률)로 알려주기"**입니다.
  이미지는 픽셀 조금만 바뀌어도 데이터가 크게 달라집니다. 그래서 분석가($\varphi$)가 "안경 수치는 정확히 80.00이야!"라고 너무 깐깐하게 정답을 내리면 모델이 학습하다가 고장 나거나 외워버립니다(과적합).
  그래서 꼼수를 씁니다. 분석가에게 **"안경 수치는 대략 75에서 85 사이야(평균과 표준편차)"**라고 범위를 알려주게 합니다. 그러면 그 **범위 안에서 아무 숫자나 하나 랜덤으로 뽑아서(랜덤 추출)** 화가에게 줍니다.
  신기하게도 이렇게 '약간의 랜덤성'을 섞어주면, 인공지능이 이미지를 훨씬 더 부드럽고 자연스럽게(유연하게) 학습하게 됩니다.

### 6. 결론: 학습이 끝난 후

- **어려운 설명:** 학습을 마친 후에는 디코더 모델 $\theta$만 사용하여 변하는 잠재 벡터 $z$에 따라 변화하는 이미지를 확인.
- **쉬운 설명:** **"분석가는 해고, 화가만 데리고 마음대로 그림 그리기"**입니다.
  이 과정을 무수히 반복하면(분석가가 요약 $\rightarrow$ 랜덤 추출 $\rightarrow$ 화가가 그리기 $\rightarrow$ 원본과 비교), 마침내 화가(디코더 $\theta$)는 주문서($z$)를 해석해서 그림을 그리는 완벽한 마스터가 됩니다.
  학습이 다 끝나면 분석가(인코더 $\varphi$)는 더 이상 필요 없습니다. 이제 우리가 직접 주문서(잠재 벡터 $z$)의 숫자를 이리저리 바꾸면서 화가(디코더 $\theta$)에게 주면, 화가는 그 숫자에 맞춰 안경을 씌웠다 벗겼다, 웃게 했다 울게 했다 하며 새로운 이미지를 마음대로 생성해 냅니다.

---

### 💡 한 줄 요약

**"VAE는 이미지를 숫자로 압축하는 '인코더'와, 그 숫자를 다시 이미지로 복원하는 '디코더'를 만드는데, 이 과정에서 숫자 대신 '확률 범위(랜덤)'를 사용하여 인공지능이 새롭고 자연스러운 이미지를 만들어낼 수 있도록 학습시키는 모델이다."**
